#!/usr/bin/env python3
"""
Generate one SLURM file per question that sweeps message size, node count,
and processes-per-node. Output is appended directly to length.txt, nodes.txt,
and processes.txt in runs/<question>/.

Usage:
  python generate.py q1
  python generate.py q1 --length 3 19 --nodes 1 10 --processes 1 4
"""
...
# !! SET THIS to your project path on the HPC !!
HPC_BASE_PATH = "/home/kanwarveer/lab-4"

# Defaults when sweeping other parameters
DEFAULT_MSG_SIZE = 16384
DEFAULT_NODES = 10
DEFAULT_PPN = 1

# Max allocation (must cover all sweep values)
MAX_NODES = 10
MAX_PPN = 4
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate SLURM benchmark file")
    parser.add_argument("question", help="e.g. q1, q2, q3, q4")
    ...
    out_dir = os.path.join(args.output_dir, args.question)
    os.makedirs(out_dir, exist_ok=True)
    path = os.path.join(out_dir, "bench.slurm")
    write_slurm(path, generate(args.question, length_vals, node_vals, proc_vals))
