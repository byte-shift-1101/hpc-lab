...
def plot(x, blocking_y, nonblocking_y, mpi_y, xlabel, ylabel="Time (s)", title="", outfile=None, log_axes=False):
    plt.figure()
    plt.plot(x, blocking_y, marker="o", color="blue", label="Algorithm 1")
    plt.plot(x, nonblocking_y, marker="s", color="red", label="Algorithm 2")
    plt.plot(x, mpi_y, marker="^", color="green", label="Algorithm 3")
    if log_axes:
